...
def flow_warp(feature, flow, mask=False, mode="bilinear", padding_mode="zeros"):
    ...
    return bilinear_sample(
        feature, grid, mode=mode, padding_mode=padding_mode, return_mask=mask
    )
...
class DDIMScheduler(SchedulerMixin, ConfigMixin):
    ...
    def step(
        ...
        fwd_mask=None,
        bwd_mask=None,
        latent_expand_l=None,
        latent_expand_r=None,
        latent_expand_t=None,
        latent_expand_b=None,
        latent_height=None,
        latent_width=None,
        gt=None,
        ...
        if bwd_flow is not None and gt is not None:
            pred_original_sample[
                ...,
        ...
        if bwd_flow is not None:
            video_length = pred_original_sample.shape[2]
            pred_original_sample = rearrange(
                pred_original_sample, "b c f h w -> (b f) c h w"
            )
            bwd_mask = F.interpolate(bwd_mask.unsqueeze(1), scale_factor=1 / 2).squeeze(
                1
            )
            for _ in range(1):  # merge one time
                pred_original_sample[1:] = (
                    flow_warp(pred_original_sample[:-1], bwd_flow)
                    * bwd_mask.unsqueeze(1)
                    + (1 - bwd_mask.unsqueeze(1)) * pred_original_sample[1:]
                )
            pred_original_sample = rearrange(
        ...
        if fwd_flow is not None:
            ...
            fwd_mask = F.interpolate(fwd_mask.unsqueeze(1), scale_factor=1 / 2).squeeze(
                1
            )
            for _ in range(1):  # merge one time
                pred_original_sample[:-1] = (
                    flow_warp(pred_original_sample[1:], fwd_flow)
                    * fwd_mask.unsqueeze(1)
                    + (1 - fwd_mask.unsqueeze(1)) * pred_original_sample[:-1]
                )
            pred_original_sample = rearrange(
                pred_original_sample, "(b f) c h w -> b c f h w", f=video_length
            )
