...
def main(base_config, config):
    ...
    if accelerator.is_main_process:
        ...
        OmegaConf.save(
            base_config, os.path.join(base_config.output_dir, "base_config.yaml")
        )
    ...
    noise_scheduler = DDPMScheduler(
        **OmegaConf.to_container(base_config.noise_scheduler_kwargs)
    )
    tokenizer = CLIPTokenizer.from_pretrained(
        base_config.model_config.sd_path, subfolder="tokenizer"
    )
    text_encoder = CLIPTextModel.from_pretrained(
        base_config.model_config.sd_path, subfolder="text_encoder"
    )
    vae = AutoencoderKL.from_pretrained(
        base_config.model_config.sd_path, subfolder="vae"
    )
    unet = UNet3DConditionModel.from_pretrained_2d(
        base_config.model_config.sd_path, subfolder="unet"
    )
    ...
    if (
        base_config.model_config.control_path is not None
        and base_config.model_config.control_path != ""
    ):
        controlnet = ControlNetModel.from_pretrained_2d(
            base_config.model_config.control_path
        )

    if (
        base_config.model_config.temporal_path is not None
        and base_config.model_config.temporal_path != ""
    ):
        motion_module_state_dict = torch.load(
            base_config.model_config.temporal_path, map_location="cpu"
        )
    ...
    for name, module in unet.named_modules():
        if (
            base_config.training_config.trainable_modules is not None
            and name.endswith(tuple(base_config.training_config.trainable_modules))
            and "controlnet" not in name
        ):
            print(name)
            tmp = []
    ...
    if base_config.training_config.scale_lr:
        learning_rate = (
            learning_rate
            * base_config.training_config.gradient_accumulation_steps
            * base_config.training_config.train_batch_size
    ...
    optimizer = optimizer_cls(
        ...
        betas=(
            base_config.training_config.adam_beta1,
            base_config.training_config.adam_beta2,
        ),
    ...
    train_dataset = TuneAVideoDataset(**config.train_data)
    ...
    lr_scheduler = get_scheduler(
        ...
        optimizer=optimizer,
        num_warmup_steps=base_config.training_config.lr_warmup_steps
        * base_config.training_config.gradient_accumulation_steps,
        num_training_steps=base_config.training_config.max_train_steps
        * base_config.training_config.gradient_accumulation_steps,
    )
    ...
    num_update_steps_per_epoch = math.ceil(
        len(train_dataloader) / base_config.training_config.gradient_accumulation_steps
    )
    # Afterwards we recalculate our number of training epochs
    num_train_epochs = math.ceil(
        base_config.training_config.max_train_steps / num_update_steps_per_epoch
    )
    ...
    logger.info(
        f"  Instantaneous batch size per device = {base_config.training_config.train_batch_size}"
    )
    logger.info(
        f"  Total train batch size (w. parallel, distributed & accumulation) = {total_batch_size}"
    )
    logger.info(
        f"  Gradient Accumulation steps = {base_config.training_config.gradient_accumulation_steps}"
    )
    logger.info(
        f"  Total optimization steps = {base_config.training_config.max_train_steps}"
    )
    global_step = 0
    first_epoch = 0
    ...
    progress_bar = tqdm(
        range(global_step, base_config.training_config.max_train_steps),
    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet, loras):
                ...
                if latents is None:
                    ...
                    for idx, (
                        ...
                        zip(
                            ...
                            config.validation_data.prompts_l,
                            config.validation_data.prompts_r,
                            config.validation_data.prompts_t,
                            config.validation_data.prompts_b,
                        ...
                        with get_autocast_context(accelerator.device):
                            sample_wide = validation_pipeline(
                                ...
                                init_video=pixel_values,
                                **config.validation_data,
                            ).videos
                        samples.append(sample_wide)
                    samples = torch.concat(samples)
                    save_path = f"{base_config.output_dir}/samples/sample-baseline-wocontrol.gif"
                    save_videos_grid(samples, save_path)
                    logger.info(f"Saved samples to {save_path}")

                    unet.controlnet_scale = base_config.model_config.control_scale
                    # Validation for basic use of inpainting block and temporal block without input-specific adaptation
                    samples = []
                    for idx, (
                        prompt,
                        prompt_l,
                        ...
                        samples.append(sample_wide)
                    samples = torch.concat(samples)
                    save_path = f"{base_config.output_dir}/samples/sample-baseline.gif"
                    save_videos_grid(samples, save_path)
                    logger.info(f"Saved samples to {save_path}")
                ...
                model_pred = unet(
                    noisy_latents, timesteps, encoder_hidden_states, control=control
                ).sample
                ...
                avg_loss = accelerator.gather(
                    loss.repeat(base_config.training_config.train_batch_size)
                ).mean()
                train_loss += (
                    avg_loss.item()
                    / base_config.training_config.gradient_accumulation_steps
                ...
                if accelerator.sync_gradients:
                    accelerator.clip_grad_norm_(
                        trainable_params, base_config.training_config.max_grad_norm
                    )
                optimizer.step()
                lr_scheduler.step()
                optimizer.zero_grad()
            ...
            if accelerator.sync_gradients:
                ...
                if global_step % base_config.training_config.checkpointing_steps == 0:
                    if accelerator.is_main_process:
                        save_path = os.path.join(
                            base_config.output_dir, f"checkpoint-{global_step}"
                        )
                ...
                if global_step % base_config.training_config.validation_steps == 0:
                    if accelerator.is_main_process:
                        ...
                        if loras is not None:
                            loras = adjust_motion_lora(
                                unet,
                                loras,
                                config.validation_data.scale_l,
                                config.validation_data.scale_r,
                                config.validation_data.scale_t,
                                config.validation_data.scale_b,
                                config.validation_data.height,
                                config.validation_data.width,
                                k=1,
                            )
                        ...
                        for idx, (
                            ...
                            samples.append(sample_wide)
                        samples = torch.concat(samples)
                        save_path = f"{base_config.output_dir}/samples/adjusted-k=1-{global_step}.gif"
                        save_videos_grid(samples, save_path)
                        logger.info(f"Saved samples to {save_path}")
                        if loras is not None:
                            loras = adjust_motion_lora(
                        ...
                        for idx, (
                            ...
                            samples.append(sample_wide)
                        samples = torch.concat(samples)
                        save_path = f"{base_config.output_dir}/samples/adjusted-k=2-{global_step}.gif"
                        save_videos_grid(samples, save_path)
                        logger.info(f"Saved samples to {save_path}")
                        if loras is not None:
                            loras = adjust_motion_lora(
                        ...
                        for idx, (
                            ...
                            samples.append(sample_wide)
                        samples = torch.concat(samples)
                        save_path = f"{base_config.output_dir}/samples/adjusted-k=1|2-{global_step}.gif"
                        save_videos_grid(samples, save_path)
                        logger.info(f"Saved samples to {save_path}")
                        if loras is not None:
                            loras = adjust_motion_lora(
...
if __name__ == "__main__":
    ...
    for base_key, base_config in base_configs.items():
        ...
        for key, config in list(configs.items())[args.start : args.end]:
            # if key not in ['c037229485', 'c0821cba97', 'c0f856e4de', 'c15ae32d78', 'c1bd023223', 'c1bfacba4a', 'c1dcd30fb2', 'c25fb8b4c4', 'c285ede7f3', 'c2a6163d39', 'c3517ebed5', 'c3aabac30c', 'c3bb62a2f7', 'c454f19e90', 'c4c410ccd7', 'c5b94822e3', 'c603a78689', 'c608df73fb', 'c64e9d1f7e', 'c682d1748f', 'c6d04b1ca3', 'd76e963754', 'd7b3892660', 'd8b3e257da', 'd8b93e6bb1', 'd90475278d', 'd949468ad6', 'd9b2b4aa86', 'da0d261f66', 'da553b619f', 'daa41653c3', 'daac20af89', 'db80f00f29', 'db8bf2430a', 'dbd729449a', 'dc0928b157', 'dc9aa0b8c0', 'dca2479b62', 'dcc0637430', 'dccdc5dfa1', 'dcd3e1b53e', 'dd0e8ec389', 'eff7c1c098', 'f00dc892b2', 'f019c9ff98', 'f01edcbffb', 'f0866da89c', 'f12eb5256e', 'f1df2ea2dc', 'f29119c644', 'f320d19ad9', 'f3419f3a62', 'f35029f76d', 'f39dc2240d', 'f3aa63fa74', 'f3b8b6e438', 'f3c6b0c0c8', 'f3f3c201bd', 'f46ccf8864', 'f4865471b4', 'f4daf9728f', 'f505ae958c']:
            # continue
            base_config.output_dir = os.path.join(
                os.path.join(base_output_dir, base_key), str(key)
            )
            # try:
